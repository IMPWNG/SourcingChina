...
After the OCR fields are written, the same command saves each company and contact in Supabase, then crawls a products page on the printed website and saves those products on that company. A matching website, or the Chinese and English names when there is no website, updates the existing row. New companies stay unpublished. If the Supabase URL or secret is missing, the card text stays in the JSON file, the site is not crawled, and the command prints the variable names. `cards:import` reloads a finished JSON file without crawling again.
...
| `MAMMOUTH_API_KEY` | Server only. Mammouth key. Card text and the site product crawl use it. |
...
- A photo-only card is read in the app with Tesseract (`tesseract.js`, English and simplified Chinese). That text is sent to Mammouth’s OpenAI-compatible API at `https://api.mammouth.ai/v1` with `MAMMOUTH_MODEL` (`gpt-4.1-nano` by default) for the company fields. If the photo has no readable text, the draft says so. Google Vision is not used. The draft is saved before any site crawl. On upload that crawl stays on the same host, at most 8 pages, and is limited to a few seconds so the request can finish. A later scrape can still use the longer bound. Mammouth then extracts products (name, description, image, category). If `MAMMOUTH_API_KEY` is missing, upload does not crash: the card keeps the OCR or pasted text and the crawl is skipped. A card with no website is not crawled. Apply `supabase/migrations/20260924143000_products.sql` before live product rows can be stored.
